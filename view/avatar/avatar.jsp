...
<script type="text/javascript">
	window.onload = function() {
		var present_tab = document.querySelector("#room-header-nav a:last-child");
...
<body>
	...
	<c:set var="myAvtList" value="${myAvtList}" scope="request"/>
	<article id="room-manage">
		<!-- 헤더 -->
		...
		<article id="room-body">
			<c:choose>
			<c:when test="${proc=='avatar'}">
				<c:import url="../avatar/avatarRaise.jsp"/>
			</c:when>
			<c:when test="${proc=='raise'}">
				<c:import url="../avatar/avatarRaisePick.jsp"/>
			</c:when>
			<c:when test="${proc=='store'}">
				<c:import url="../avatar/avatarStore.jsp"/>
			</c:when>
			</c:choose>
		</article>	
	</article>
